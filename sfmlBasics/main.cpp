...
private:
    ...
    sf::Vector2i activeCell = sf::Vector2i(-1, -1);
    virtual void draw(sf::RenderTarget& target, sf::RenderStates states)const {
...
public:
    ...
    sf::Vector2i getActiveCell() {
        return activeCell;
    }
    void colorPressedCell() {
    ...
    void mixCells() {
        ...
        for (int i = 0; i < cells.size(); ++i) {
            for (int j = 0; j < cells[i].size(); ++j) {
                cells[i][j].setValue(v[i + j + 3*i].getValue());
            }
        }  
    }
    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(event)) {
            ...
            if (event.type == sf::Event::MouseButtonPressed) {
                ...
                board.setActiveCell(cellCords.x, cellCords.y);
             //   std::cout << cellCords.x << ' ' << cellCords.y << "\n";
                if (board.getActiveCell() == sf::Vector2i(-1, -1)) {
                    break;
                }
              
                board.move(cellCords.x, cellCords.y);
            }
